...
void main() {
  test(
    'playbackHistoryReadStateProvider keeps latest entry but marks it non resumable when near end',
    () async {
      ...
      final historyRepo = _FakeHistoryLocalRepository(entry: historyEntry);
      ...
      final state = await container.read(
        playbackHistoryReadStateProvider((
          contentId: 'movie-1',
          type: ContentType.movie,
        )).future,
      );
      ...
      expect(state.entry, same(historyEntry));
      expect(state.resumableEntry, isNull);
      expect(state.resumeResolution.canResume, isFalse);
      expect(state.resumeResolution.reasonCode, ResumeReasonCode.nearEnd);
      expect(latestEntry, same(historyEntry));
    },
  );

  test(
    'inProgressHistoryEntryProvider excludes entries that are near-end non resumable',
    () async {
      final getIt = GetIt.asNewInstance();
  ...
  test(
    'inProgressHistoryEntryProvider suppresses resumable series when a manual seen state exists',
    () async {
      final getIt = GetIt.asNewInstance();
      final historyEntry = HistoryEntry(
        contentId: 'series-1',
