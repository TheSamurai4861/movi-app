...
def main():
    ...
    parser.add_argument("--lib", default="lib", help="Dossier racine (par défaut: lib)")
    parser.add_argument("--assets", default="assets", help="Dossier assets à afficher (par défaut: assets)")
    parser.add_argument("--output", "-o", default="lib_outline.txt", help="Fichier de sortie OUTLINE (par défaut: lib_outline.txt)")
    ...
    parser.add_argument("--index-json", action="store_true", default=True, help="Écrire <output>.index.json (défaut: True)")
    parser.add_argument("--no-index-json", action="store_true", help="Désactiver l’écriture du JSON d’index")

    parser.add_argument("--methods-in-classes", action="store_true",
                        help="Inclure les méthodes à l'intérieur des classes (sans contenu)")

    # --- Nouveaux arguments pour l'export de contenu ---
