...
| SYNC-REQ-006 | La sheet sous-titres expose uniquement la selection de piste + bouton `settings.svg` vers la page paramètres (pas de quick settings). | C2/L2 | `lib/src/features/player/presentation/widgets/track_selection_menu.dart`, `lib/src/features/player/presentation/pages/video_player_page.dart` |
| SYNC-REQ-007 | Le rendu des sous-titres reste visible (zone non trop basse) en lecture plein écran avec/sans contrôles. | C1/L1 | `lib/src/features/player/presentation/pages/video_player_page.dart` |
...
| SYNC-REQ-006 | `test/features/player/presentation/widgets/track_selection_menu_test.dart` | Absence quick settings + présence action settings |
| SYNC-REQ-007 | Validation manuelle + diagnostic `player_subtitle_view_configuration` | Sous-titres lisibles et non rognés en bas |
