...
## Entree `LOG-2026-04-02-056`

- **Date** : `2026-04-02`
- **Lot ID** : `PH4-LOT-006` (Phase 4 / Jalon M3 — `core/storage`)
- **Type** : phase 4 — durcissement secure storage, corruption explicite, fallback sûr et tests de migration
- **Source de besoin** : `movi_nasa_refactor_plan_v3.md` §14 (Phase 4 / `core/storage`) + `docs/roadmap/phase_4_refondation_noyau_critique.md` (Jalon M3) + risques `SYS-P1-STO-001` / `HZD-STO-001`
- **Composants produits / modifies** :
  - `lib/src/core/storage/repositories/secure_payload_store.dart`
  - `lib/src/core/storage/repositories/secure_storage_repository.dart`
  - `lib/src/core/storage/storage_failures.dart`
  - `lib/src/core/storage/services/storage_module.dart`
  - `lib/src/core/di/injector.dart`
  - `lib/src/core/parental/application/services/parental_session_service.dart`
  - `lib/src/core/diagnostics/infrastructure/identity/diagnostic_identity_hasher.dart`
  - `lib/src/features/library/application/services/cloud_sync_preferences.dart`
  - `lib/src/features/library/application/services/cloud_sync_cursor_store.dart`
  - `lib/src/features/library/application/services/library_cloud_sync_service.dart`
  - `lib/src/features/library/presentation/providers/library_cloud_sync_access_providers.dart`
  - `lib/src/features/library/presentation/providers/library_cloud_sync_providers.dart`
  - `lib/src/features/settings/presentation/widgets/export_diagnostics_sheet.dart`
  - `test/support/fake_secure_storage_platform.dart`
  - `test/core/storage/repositories/secure_storage_repository_test.dart`
  - `test/core/storage/database/sqlite_database_migrations_test.dart`
  - `test/core/parental/application/services/parental_session_service_test.dart`
  - `test/core/diagnostics/infrastructure/identity/diagnostic_identity_hasher_test.dart`
  - `test/features/library/application/services/cloud_sync_preferences_test.dart`
  - `test/features/library/application/services/cloud_sync_cursor_store_test.dart`
- **Criticite du changement** : `C1`
- **Classe principale** : `L1`
- **Decision / resultat** :
  - contrat `SecurePayloadStore` introduit pour éviter la dépendance directe des flux critiques à l’implémentation concrète ;
  - `SecureStorageRepository` détecte explicitement les payloads non-JSON / non-map et remonte des erreurs storage typées ;
  - `parental`, `diagnostics` et `cloud sync` retombent vers un état sûr (`null`, valeur par défaut, cursor initial, salt régénéré) sur corruption locale ;
  - test de migration SQLite v13→v19 ajouté sur fixture représentative (playlist settings + history + continue_watching).
- **Risques identifies** :
  - persistance locale déjà corrompue : désormais contenue mais pas “réparée” au-delà du cleanup best-effort de la clé/entrée en cause ;
  - exécution réelle des tests non confirmée dans cette session ;
  - le lot ne couvre pas encore tous les repositories SQLite du sous-domaine `core/storage`.
- **Rollback / mitigation** :
  - rollback par revert borné des fichiers `core/storage` / consommateurs listés ;
  - containment conservé : suppression best-effort des clés corrompues et fallback sûr au runtime ;
  - si régression observée, retour temporaire à la lecture brute précédente possible via revert complet du lot.
- **Preuves / validation** :
  - `docs/quality/validation_evidence_index.md` : `PH4-EVD-013..015`.
  - Exécution locale non réalisée ici : absence de SDK Flutter/Dart dans l’environnement Codex au moment du lot.
- **Derogation** : aucune.
- **Statut** : ouvert.
