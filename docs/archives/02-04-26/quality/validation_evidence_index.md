...
| **PH4-EVD-013** | `core/storage` contracts + gestion explicite des payloads corrompus (M3) | `lib/src/core/storage/repositories/secure_payload_store.dart` + `lib/src/core/storage/repositories/secure_storage_repository.dart` + `lib/src/core/storage/storage_failures.dart` + `lib/src/core/storage/services/storage_module.dart` | Contrat `SecurePayloadStore` introduit ; erreurs de lecture/écriture/corruption explicites ; secure storage durci contre JSON non conforme |
| **PH4-EVD-014** | Consommateurs critiques storage en mode dégradé sûr | `lib/src/core/parental/application/services/parental_session_service.dart` + `lib/src/core/diagnostics/infrastructure/identity/diagnostic_identity_hasher.dart` + `lib/src/features/library/application/services/cloud_sync_preferences.dart` + `lib/src/features/library/application/services/cloud_sync_cursor_store.dart` + `lib/src/features/library/application/services/library_cloud_sync_service.dart` + `lib/src/core/di/injector.dart` | `parental`, `diagnostics` et `cloud sync` retombent sur un état sûr si le secure storage est corrompu ou indisponible ; dépendance recentrée sur le contrat storage |
| **PH4-EVD-015** | Tests storage ciblés (secure storage + migration SQLite représentative) | `test/core/storage/repositories/secure_storage_repository_test.dart` + `test/core/parental/application/services/parental_session_service_test.dart` + `test/core/diagnostics/infrastructure/identity/diagnostic_identity_hasher_test.dart` + `test/features/library/application/services/cloud_sync_preferences_test.dart` + `test/features/library/application/services/cloud_sync_cursor_store_test.dart` + `test/core/storage/database/sqlite_database_migrations_test.dart` + `test/support/fake_secure_storage_platform.dart` | Tests ajoutés pour payloads corrompus, fallback sûr et migration v13→v19 ; exécution locale non réalisée dans cette session car SDK Flutter/Dart absent |
